...
import { Calendar, Check, AlertCircle, Plus, Receipt } from 'lucide-react';
...
const getNextDueDate = (bill: Bill): Date => {
    if (!bill.isRecurring) return parseISO(bill.dueDate);
    const now = new Date();
...
export const Bills: React.FC = () => {
    ...
    const { upcoming, paid, overdue, monthly, totalMonthlyBills } = useMemo(() => {
        const now = new Date();
        const thisMonth: Bill[] = [];
        ...
        data.bills.forEach(bill => {
            ...
            if (bill.isPaid) paidBills.push(bill);
            else if (isPast(dueDate) && !isToday(dueDate)) overdueBills.push(bill);
            else upcomingBills.push(bill);
            if (isThisMonth(dueDate)) thisMonth.push(bill);
        });

        const totalMonthly = thisMonth.reduce((sum, bill) => sum + bill.amount, 0);

        // Total Monthly Bills: recurring monthly + (weekly * 52 / 12)
        const recurringMonthly = data.bills
            .filter(b => b.isRecurring && b.frequency === 'monthly')
            .reduce((sum, b) => sum + b.amount, 0);
        const recurringWeekly = data.bills
            .filter(b => b.isRecurring && b.frequency === 'weekly')
            .reduce((sum, b) => sum + b.amount, 0);
        const totalMonthlyBills = recurringMonthly + (recurringWeekly * 52 / 12);
    ...
    const BillCard: React.FC<{ bill: Bill; isDue?: boolean }> = ({ bill, isDue }) => {
        ...
        return (
            <div onClick={() => setBillToEdit(bill)} className="bg-[#161618] border border-white/5 p-5 rounded-sm flex flex-col gap-3 group hover:border-white/10 transition-all cursor-pointer">
                <div className="flex items-start gap-3">
                    <div className={clsx('w-8 h-8 rounded-full flex items-center justify-center border flex-shrink-0 mt-0.5',
                        bill.isPaid ? 'border-emerald-vein/20 text-emerald-vein bg-emerald-vein/5' :
                        isDue ? 'border-magma/20 text-magma bg-magma/5' : 'border-white/10 text-white/60')}>
                ...
                <div className="flex items-end justify-between pt-2 border-t border-white/5">
                    <div className={clsx('text-[10px] font-mono font-bold uppercase',
                        bill.isPaid ? 'text-emerald-vein' : isDue ? 'text-magma' : 'text-iron-dust')}>
    ...
    return (
        <div className="p-12 max-w-7xl mx-auto h-full flex flex-col slide-up overflow-y-auto custom-scrollbar">
            <div className="flex items-end justify-between mb-12">
                ...
                <button onClick={() => setShowAddModal(true)} className="flex items-center gap-2 px-6 py-3 bg-magma text-obsidian rounded-sm text-xs font-bold uppercase tracking-wider hover:bg-magma/90 transition-colors shadow-[0_0_15px_rgba(255,77,0,0.3)]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                ...
                <div className="bg-[#161618] border border-white/5 p-6 rounded-sm">
                    <div className="flex items-center gap-2 mb-4">
                        <Receipt size={16} className="text-magma" />
                        <span className="font-mono text-[10px] text-iron-dust uppercase tracking-[2px]">Total Monthly Bills</span>
                    </div>
                    <div className="text-3xl font-bold text-white">
                        {currencySymbol}{totalMonthlyBills.toLocaleString('en-GB', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </div>
                    <div className="text-[10px] font-mono text-iron-dust mt-1">Recurring avg / month</div>
                </div>
                ...
                <div className="bg-[#161618] border border-magma/20 p-6 rounded-sm">
                    ...
                    <div className="text-3xl font-bold text-magma">{overdue.length}</div>
                </div>
            </div>

            {/* Tabs */}
            <div className="flex gap-2 mb-8 border-b border-white/5">
                <button onClick={() => setShowUpcoming(true)} className={clsx('px-4 py-3 border-b-2 font-mono text-xs font-bold uppercase tracking-wider transition-colors', showUpcoming ? 'border-magma text-white' : 'border-transparent text-iron-dust hover:text-white')}>Upcoming</button>
                <button onClick={() => setShowUpcoming(false)} className={clsx('px-4 py-3 border-b-2 font-mono text-xs font-bold uppercase tracking-wider transition-colors', !showUpcoming ? 'border-emerald-vein text-white' : 'border-transparent text-iron-dust hover:text-white')}>Paid</button>
            </div>
                        ...
                        {overdue.length > 0 && (
                            <div className="mb-8">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                                    {overdue.map(bill => <BillCard key={bill.id} bill={bill} isDue={true} />)}
                                </div>
                            </div>
                        )}
                        {upcoming.length > 0 ? (
                            <div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                                    {upcoming.map(bill => <BillCard key={bill.id} bill={bill} />)}
                                </div>
                            </div>
                        ) : (
                            <div className="text-center py-12 text-iron-dust font-mono text-sm">No upcoming bills</div>
                        )}
                    </>
                ) : (
                    <>
                        {paid.length > 0 ? (
                            ...
                            <div className="text-center py-12 text-iron-dust font-mono text-sm">No paid bills yet</div>
                        )}
                    </>
                )}
            </div>

            <AddBillModal isOpen={showAddModal || !!billToEdit} onClose={() => { setShowAddModal(false); setBillToEdit(null); }} billToEdit={billToEdit || undefined} />
        </div>
    );
};
