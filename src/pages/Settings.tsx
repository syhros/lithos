...
import type { CardSortOrder, SortOrders } from '../context/FinanceContext';
...
const DELETE_OPTIONS: SelectGroup[] = [
  { options: [{ value: 'none', label: 'Select deletion type\u2026' }] },
  { label: 'Transactions', options: [{ value: 'all_transactions', label: 'All Transactions', hint: 'removes every transaction record' }, { value: 'recent_transactions', label: 'Recent Transactions', hint: 'delete last X months of transactions' }] },
  { label: 'Assets & Debts', options: [{ value: 'accounts', label: 'All Accounts', hint: 'resets all account balances to \u00a30' }, { value: 'debts', label: 'All Debts', hint: 'removes all debt records' }] },
  { label: 'Other Data', options: [{ value: 'investments', label: 'All Investments', hint: 'clears investment transactions & holdings' }, { value: 'bills', label: 'All Bills', hint: 'removes all bill & subscription records' }] },
  { label: 'Danger Zone', options: [{ value: 'factory_reset', label: 'Factory Reset', hint: 'deletes everything \u2014 irreversible' }] },
];

const SORT_OPTIONS: { value: CardSortOrder; label: string }[] = [
  { value: 'highest', label: 'Highest \u2192 Lowest' },
  { value: 'az',      label: 'A \u2192 Z' },
  { value: 'date',    label: 'Date Added' },
];

const PAGE_KEYS: { key: keyof SortOrders; label: string }[] = [
...
export const Settings: React.FC = () => {
  ...
  const { data, refreshData, addAccount, addTransaction, addDebt, addBill, sortOrders, updateSortOrder } = useFinance();
  ...
  const handleImportJSON = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setImportFileName(file.name); setImportPhase('parsing'); setImportMessage('Reading file\u2026');
    let parsed: any;
    try { const text = await file.text(); parsed = JSON.parse(text); }
    catch { setImportPhase('error'); setImportMessage('Invalid JSON file \u2014 could not parse.'); return; }
    ...
    if (!hasTransactions && !hasAssets && !hasDebts && !hasBills) {
      setImportPhase('error'); setImportMessage('File does not look like a Lithos backup.'); return;
    }
    setImportPhase('importing');
    let imported = { accounts: 0, transactions: 0, debts: 0, bills: 0 };
    let errors: string[] = [];
    if (hasAssets) { for (const a of parsed.assets) { try { await addAccount({ name: a.name ?? 'Imported Account', type: a.type ?? 'checking', currency: a.currency ?? 'GBP', institution: a.institution ?? '', color: a.color ?? '#ffffff', startingValue: typeof a.startingValue === 'number' ? a.startingValue : 0, interestRate: a.interestRate, symbol: a.symbol, isClosed: a.isClosed ?? false, openedDate: a.openedDate, closedDate: a.closedDate }); imported.accounts++; } catch (err: any) { errors.push(`Account "${a.name}": ${err?.message ?? 'unknown error'}`); } } }
    if (hasDebts) { for (const d of parsed.debts) { try { await addDebt({ name: d.name ?? 'Imported Debt', type: d.type ?? 'credit_card', limit: typeof d.limit === 'number' ? d.limit : 0, apr: typeof d.apr === 'number' ? d.apr : 0, minPaymentType: d.minPaymentType ?? 'fixed', minPaymentValue: typeof d.minPaymentValue === 'number' ? d.minPaymentValue : 0, startingValue: typeof d.startingValue === 'number' ? d.startingValue : 0, promo: d.promo }); imported.debts++; } catch (err: any) { errors.push(`Debt "${d.name}": ${err?.message ?? 'unknown error'}`); } } }
    if (hasBills) { for (const b of parsed.bills) { try { await addBill({ name: b.name ?? 'Imported Bill', amount: typeof b.amount === 'number' ? b.amount : 0, dueDate: b.dueDate ?? b.due_date ?? new Date().toISOString().split('T')[0], isPaid: b.isPaid ?? false, autoPay: b.autoPay ?? false, category: b.category ?? 'Other', isRecurring: b.isRecurring ?? false, frequency: b.frequency, recurringEndDate: b.recurringEndDate }); imported.bills++; } catch (err: any) { errors.push(`Bill "${b.name}": ${err?.message ?? 'unknown error'}`); } } }
    if (hasTransactions) { for (const t of parsed.transactions) { try { await addTransaction({ date: t.date ?? new Date().toISOString().split('T')[0], description: t.description ?? '', amount: typeof t.amount === 'number' ? t.amount : 0, type: t.type ?? 'expense', category: t.category ?? 'Other', accountId: t.accountId ?? t.account_id ?? undefined, notes: t.notes, symbol: t.symbol, quantity: t.quantity, price: t.price, currency: t.currency }); imported.transactions++; } catch (err: any) { errors.push(`Transaction "${t.description}": ${err?.message ?? 'unknown error'}`); } } }
    if (errors.length === 0) {
      setImportPhase('done');
  ...
  return (
    <div className="p-12 max-w-4xl mx-auto h-full flex flex-col slide-up overflow-y-auto custom-scrollbar">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 my-12">
        {[['Transactions', stats.txCount], ['Accounts', stats.accountCount], ['Debts', stats.debtCount], ['Bills', stats.billCount]].map(([label, val]) => (
          <div key={label} className="bg-[#161618] border border-white/5 p-4 rounded-sm">
            <div className="text-[10px] font-mono text-iron-dust uppercase tracking-[2px] mb-2">{label}</div>
      ...
      <div className="space-y-8">
        ...
        <div className="border border-white/5 rounded-sm p-8 bg-[#161618]">
          ...
          {SORT_OPTIONS.map(opt => (
            <div key={opt.value} className="grid grid-cols-[120px_1fr_1fr_1fr] gap-3 mb-2 items-center">
              ...
              {PAGE_KEYS.map(p => {
                const active = sortOrders[p.key] === opt.value;
                return (
                  <button
                    key={p.key}
                    onClick={() => updateSortOrder(p.key, opt.value)}
                    className={clsx(
                      'h-9 rounded-sm border transition-all text-[10px] font-bold uppercase tracking-wider',
                      active
                        ? 'border-magma/60 bg-magma/15 text-magma'
        ...
        <div className="border border-white/5 rounded-sm p-8 bg-[#161618]">
          ...
          <div className="space-y-4">
            <div><p className="text-sm text-iron-dust mb-3">Export all your financial data as CSV for use in spreadsheets</p><button onClick={handleExportCSV} className="flex items-center gap-2 px-6 py-3 bg-white/10 border border-white/20 text-white rounded-sm text-sm font-bold uppercase tracking-wider hover:bg-white/15 transition-colors"><Download size={14} /> Export as CSV</button></div>
            <div><p className="text-sm text-iron-dust mb-3">Export complete backup as JSON for archiving or migration</p><button onClick={handleExportJSON} className="flex items-center gap-2 px-6 py-3 bg-white/10 border border-white/20 text-white rounded-sm text-sm font-bold uppercase tracking-wider hover:bg-white/15 transition-colors"><Download size={14} /> Export as JSON</button></div>
          </div>
        </div>
        ...
        <div className="border border-white/5 rounded-sm p-8 bg-[#161618]">
          ...
          <div className="space-y-8">
            <div>
              ...
              <label className={clsx('flex items-center gap-2 px-6 py-3 rounded-sm text-sm font-bold uppercase tracking-wider transition-colors cursor-pointer w-fit border',
                ...
                importPhase === 'error' ? 'bg-red-900/20 border-red-900/30 text-red-400 hover:bg-red-900/30' : 'bg-white/10 border-white/20 text-white hover:bg-white/15')}>
                {importPhase === 'importing' || importPhase === 'parsing' ? <Loader2 size={14} className="animate-spin" /> : importPhase === 'done' ? <CheckCircle2 size={14} /> : importPhase === 'error' ? <XCircle size={14} /> : <Upload size={14} />}
              ...
              {importMessage && (
                <div className={clsx('mt-3 flex items-start gap-2 text-sm font-mono rounded-sm px-4 py-3 border', importPhase === 'done' ? 'bg-emerald-vein/5 border-emerald-vein/20 text-emerald-vein' : importPhase === 'error' ? 'bg-red-900/10 border-red-900/30 text-red-400' : 'bg-white/5 border-white/10 text-white/70')}>
        ...
        <div className="border border-red-900/30 rounded-sm p-8 bg-red-900/5">
          ...
          <div className="space-y-4">
            ...
            {deleteType === 'recent_transactions' && (<div><label className="block text-[10px] font-mono text-iron-dust uppercase tracking-[2px] mb-2">Months to Delete</label><input type="number" min="1" max="120" value={monthsToDelete} onChange={e => setMonthsToDelete(parseInt(e.target.value))} className="w-full bg-black/20 border border-white/10 p-3 text-sm text-white rounded-sm focus:border-magma outline-none font-mono" /></div>)}
            {deleteType !== 'none' && deleteType !== 'factory_reset' && (<div className="bg-yellow-900/10 border border-yellow-900/30 rounded-sm p-4 flex gap-3"><AlertCircle size={16} className="text-yellow-600 flex-shrink-0 mt-0.5" /><div className="text-sm text-yellow-600">This will permanently delete the selected data. Make sure you have a backup.</div></div>)}
            {deleteType === 'factory_reset' && (<div className="bg-red-900/20 border border-red-900/50 rounded-sm p-4 flex gap-3"><AlertCircle size={16} className="text-red-500 flex-shrink-0 mt-0.5" /><div className="text-sm text-red-400">This will delete ALL your financial data. This action is permanent and cannot be undone.</div></div>)}
            {deleteType !== 'none' && (<div className="flex gap-3">{!confirmDelete ? (<button onClick={() => setConfirmDelete(true)} className="px-6 py-3 bg-red-900/20 border border-red-900/30 text-red-400 rounded-sm text-sm font-bold uppercase tracking-wider hover:bg-red-900/30 transition-colors">Confirm Delete</button>) : (<><button onClick={() => setConfirmDelete(false)} className="px-6 py-3 bg-white/10 border border-white/20 text-white rounded-sm text-sm font-bold uppercase tracking-wider hover:bg-white/15 transition-colors">Cancel</button><button onClick={handleDelete} className="px-6 py-3 bg-red-600 border border-red-600 text-white rounded-sm text-sm font-bold uppercase tracking-wider hover:bg-red-700 transition-colors">Delete Permanently</button></>)}</div>)}
          </div>
        </div>

        {deleteSuccess && (<div className="bg-emerald-900/10 border border-emerald-900/30 rounded-sm p-4 flex gap-3 animate-in fade-in"><Check size={16} className="text-emerald-400 flex-shrink-0 mt-0.5" /><div className="text-sm text-emerald-400">Data deleted successfully</div></div>)}
      </div>
    </div>
  );
};
