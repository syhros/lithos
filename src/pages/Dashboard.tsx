...
const GridBox: React.FC<{
    ...
    history: any[]; dataKey: string; currencySymbol: string;
    plValue?: number; plPercent?: number;
...
export const Dashboard: React.FC = () => {
  ...
  const accountsPL = useMemo(() => {
    ...
    const plValue = (totalCheckingBalance + totalSavingsBalance) - totalStarting;
    return { plValue, plPercent: totalStarting > 0 ? (plValue / totalStarting) * 100 : 0 };
                        ...
                        <div className="flex bg-[#1a1c1e] rounded-sm p-1 border border-white/5">
                            {(['1W','1M','1Y'] as const).map(range => (
                                <button key={range} onClick={() => setTimeRange(range)}
                                    className={`px-4 py-1.5 text-[10px] font-mono font-bold rounded-sm transition-all ${timeRange === range ? 'bg-white text-black' : 'text-iron-dust hover:text-white'}`}>{range}</button>
                            ))}
                        </div>
                    </div>
